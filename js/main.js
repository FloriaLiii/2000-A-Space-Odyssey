/* ============================================
   LIKANGJUE星际漫游指南 - Main JavaScript
   粒子系统 + 星座连线 + 滚动动画
   ============================================ */
...
(function () {
  ...
  class VoyageAnimator {
    constructor(section) {
      ...
      this.nodes.forEach((node) => {
        this.triggers.push(
          parseFloat(node.getAttribute("data-trigger") || 0.5),
        );
      });
    ...
    startAnimation() {
      ...
      const animate = (now) => {
        ...
        const containerRect = this.section
          .querySelector(".voyage-container")
        ...
        (entries) => {
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              ...
              if (
                entry.target.classList.contains("section-voyage") &&
                !this.animated.has("voyage")
              ) {
  ...
  class HobbyGallery {
    constructor() {
      ...
      fetch("https://img.likangjue.space/assets/hobbies/photos.json?v=" + Date.now())
        ...
        .catch(() => {
          console.warn("photos.json 未找到，请运行 bash generate-photos.sh");
        });
  ...
  function drawHobbyOrbits() {
    ...
    const innerR = Math.sqrt(
      (green.x - innerCx) * (green.x - innerCx) +
        (green.y - innerCy) * (green.y - innerCy),
    );
    ...
    const outerR = Math.sqrt(
      (blue.x - outerCx) * (blue.x - outerCx) +
        (blue.y - outerCy) * (blue.y - outerCy),
    );
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (contactCanvas) {
      new ParticleSystem(contactCanvas, {
        container: contactCanvas.parentElement,
      });
    }
    ...
    (function initGisDashboard() {
      ...
      var MARKER_POINTS = [
        [22.555, 113.93, "survey"],
        [22.565, 113.96, "car"],
        [22.545, 113.95, "person"],
        [22.57, 114.04, "property"],
        [22.555, 114.07, "survey"],
        [22.545, 114.06, "car"],
        [22.565, 114.12, "person"],
        [22.555, 114.14, "property"],
        [22.535, 114.13, "survey"],
        [22.59, 113.88, "car"],
        [22.595, 113.91, "person"],
        [22.575, 113.9, "property"],
        [22.635, 114.03, "survey"],
        [22.625, 114.06, "car"],
        [22.615, 114.05, "person"],
      ];
      ...
      function clearMapOverlays() {
        ...
        if (map && map._extraLayers) {
          map._extraLayers.forEach(function (l) {
            map.removeLayer(l);
          });
          map._extraLayers = [];
        }
      ...
      function clearAnimations() {
        animTimeout.forEach(function (t) {
          clearTimeout(t);
        });
        animTimeout = [];
        clearMapOverlays();
        ...
        if (tiles.length)
          tiles.forEach(function (t) {
            t.classList.remove("loaded", "highlight");
          });
        svgRegions.forEach(function (r) {
          r.classList.remove("drawn", "filled");
        });
        markersEl.innerHTML = "";
        ...
        if (phase === "migrate") {
          if (mapReady) {
            ...
            function drawPreview() {
              if (drawPolyline) {
                overlayGroup.removeLayer(drawPolyline);
                drawPolyline = null;
              }
              if (drawPolygonLayer) {
                overlayGroup.removeLayer(drawPolygonLayer);
            ...
            function clearDraw() {
              drawPoints = [];
              drawMarkers.forEach(function (m) {
                overlayGroup.removeLayer(m);
              });
              drawMarkers = [];
              if (drawPolyline) {
                overlayGroup.removeLayer(drawPolyline);
            ...
            function onDrawClick(e) {
              ...
              if (drawPoints.length >= 3) {
                ...
                mHint.textContent =
                  "已添加 " + drawPoints.length + " 点，至少需要 3 点";
              }
            }
          ...
          if (mapReady) {
            ...
            function drawAllPolygons(interactiveMode) {
              ...
              splitPolygons.forEach(function (verts, i) {
                ...
                var poly = L.polygon(verts, {
                  color: isSelected
                    ? "#fff"
                    : splitColors[i % splitColors.length],
                ...
                if (interactiveMode) {
                  (function (idx) {
                    poly.on("click", function () {
                      ...
                      hint.textContent =
                        "已选 " + mergeSelected.length + " 区，点击确认合并";
                    });
                  })(i);
                }
                overlayGroup.addLayer(poly);
            ...
            function splitOnePolygon(verts, p1, p2) {
              ...
              poly1.push(i2.pt);
              var poly2 = [i2.pt];
              for (var k = i2.edge + 1; k < verts.length; k++)
                poly2.push(verts[k]);
            ...
            function exitSplitMode() {
              ...
              map.off("mousemove");
              if (previewLine) {
                overlayGroup.removeLayer(previewLine);
                previewLine = null;
              }
              // 移除起点标记
              overlayGroup.eachLayer(function (l) {
                if (
                  ...
                  l.options.fillColor === "#fff"
                )
                  overlayGroup.removeLayer(l);
              });
              var tb = viewport.querySelector(".gis-sdk-toolbar");
              if (tb) {
                tb.querySelectorAll(".gis-sdk-btn").forEach(function (b) {
                  b.classList.remove("active");
                });
              }
            }
            ...
            function onMapClick(e) {
              ...
              if (!cutStart) {
                ...
                if (didSplit) {
                  // 预览拆分结果，等待确认
                  pendingSplit = {
                    oldPolygons: splitPolygons.slice(),
                    newPolygons: newPolygons,
                  };
                  splitPolygons = newPolygons;
                  setTimeout(function () {
                    if (cutLine) {
                      overlayGroup.removeLayer(cutLine);
                      cutLine = null;
                    }
            ...
            function mergeSelectedPolygons() {
              ...
              var mergedVerts = [];
              mergeSelected.sort(function (a, b) {
                return a - b;
              });
      ...
      missionBtns.forEach(function (btn) {
        btn.addEventListener("click", function () {
          missionBtns.forEach(function (b) {
            b.classList.remove("active");
          });
          btn.classList.add("active");
    ...
    (function initRescueDashboard() {
      ...
      function initTracking() {
        ...
        var nodeData = [
          ...
          { x: 0.3, y: 0.3, label: "Collect", color: "#f6794d" },
      ...
      function drawTracking() {
        ...
        if (frameCount % 8 === 0) {
          ...
          particles.push({
            ...
            color: ["#4dc9f6", "#f6794d", "#67d49e", "#fbbf24"][
      ...
      function drawMessage() {
        ...
        if (msgTimer % 45 === 0) {
          var tmpl =
            MSG_TEMPLATES[Math.floor(Math.random() * MSG_TEMPLATES.length)];
          msgList.unshift({
            text: tmpl.text,
            type: tmpl.type,
            icon: tmpl.icon,
      ...
      function drawSecurity() {
        ...
        ctx.fillText(
          lockIcon +
            " TLS 1.3 ACTIVE  |  THREATS BLOCKED: " +
            (1200 + Math.floor(frameCount / 3)),
          w - 12,
          h - 12,
        );
      }
      ...
      var rescueObserver = new IntersectionObserver(
        function (entries) {
    ...
    (function initAboutSection() {
      ...
      var aboutPosObserver = new IntersectionObserver(
        function (entries) {
          entries.forEach(function (entry) {
            if (entry.isIntersecting) {
              requestAnimationFrame(() => {
